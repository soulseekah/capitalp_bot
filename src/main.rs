...
fn main() {
	...
	let key = match env::var_os("BOTKEY") {
		Some(result) => String::from(result.to_str().unwrap()),
		None => {
			println!("BOTKEY is not defined in the environment. Exiting.");
			return
		}
	};

	let botkey = ["bot", key.as_str()].join("");
	let apiurl = "https://api.telegram.org";

	let mut offset = match env::var_os("OFFSET") {
		Some(result) => String::from(result.to_str().unwrap()),
		None => String::from("0")
	};
